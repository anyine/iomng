...
public class LeaveService implements ILeaveService {
	...
	public void add(Leave leave) {
		...
		String message=person.getName()+" 有一个外出申请，"
				+ "外出时间："
				+DateFormatUtils.format(leave.getStartTime(),"yyyy-MM-dd HH:mm")
				+"归队时间："+DateFormatUtils.format(leave.getEndTime(),"yyyy-MM-dd HH:mm")
				+"事由："+leave.getReason();
		//如果已批准，则当前登录人为审批人
				if(leave.getAgree()){
					...
					message+="。可回复编号:"+leave.getId()+"*审批代码 对申请进行审批。审批代码：0 -- 不同意，1 -- 同意。系统时间：+"+DateFormatUtils.format(new Date(), "yyyy-MM-dd HH:mm");
				}
	...
	public void approve(Leave leave, User user, Boolean agree) {
		...
		String success="已被"+user.getRealname()+"审批通过了。您在"
		+DateFormatUtils.format(leave.getStartTime(), "yyyy-MM-dd HH-mm")+"至"
		+DateFormatUtils.format(leave.getEndTime(), "yyyy-MM-dd HH:mm")
		+"可刷卡自由出入而不记录违规。";
		String fail="没有通过。可致电"+user.getPhone()+"询问原因。";
		if(StringUtils.isNotBlank(phoneNumber)){
			smsService.sendMessage(phoneNumber, "您于"
		+DateFormatUtils.format(leave.getCreateTime(),"yyyy-MM-dd HH:mm")
